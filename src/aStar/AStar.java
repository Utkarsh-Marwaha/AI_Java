...
public class AStar {
	...
	private void initializeSearchSpace() {
		...
		this.searchSpace[1][7].setObstacle(true);
		this.searchSpace[2][3].setObstacle(true);
		this.searchSpace[2][4].setObstacle(true);
		this.searchSpace[2][5].setObstacle(true);
		this.searchSpace[2][6].setObstacle(true);
		this.searchSpace[2][7].setObstacle(true);
	...
	public void search() {
		...
		startNode.setH_val(manhattanHeuristic(startNode, finalNode));
	...
	private List<Node> getAllNeigbors(Node node) {
		...
		if(row-1>=0 && !this.searchSpace[row-1][col].isObstacle()) {
			searchSpace[row-1][col].setG_val(node.getG_val()+Constants.HORIZONTAL_VERTICAL_COST);
			searchSpace[row-1][col].setH_val(manhattanHeuristic(searchSpace[row-1][col], finalNode));
			neighbors.add(this.searchSpace[row-1][col]);
		}
		//block below
		if(row+1<Constants.NUM_ROWS && !this.searchSpace[row+1][col].isObstacle()) {
			searchSpace[row+1][col].setH_val(manhattanHeuristic(searchSpace[row+1][col], finalNode));
			searchSpace[row+1][col].setG_val(node.getG_val()+Constants.HORIZONTAL_VERTICAL_COST);
			neighbors.add(this.searchSpace[row+1][col]);
		}
		...
		if(col-1>=0 && !this.searchSpace[row][col-1].isObstacle()) {
			searchSpace[row][col-1].setH_val(manhattanHeuristic(searchSpace[row][col-1], finalNode));
			searchSpace[row][col-1].setG_val(node.getG_val()+Constants.HORIZONTAL_VERTICAL_COST);
			neighbors.add(this.searchSpace[row][col-1]);
		}
		//block to the right
		if(col+1<Constants.NUM_COLS && !this.searchSpace[row][col+1].isObstacle()) {
			searchSpace[row][col+1].setH_val(manhattanHeuristic(searchSpace[row][col+1], finalNode));
			searchSpace[row][col+1].setG_val(node.getG_val()+Constants.HORIZONTAL_VERTICAL_COST);
			neighbors.add(this.searchSpace[row][col+1]);
		}
	...
	public int manhattanHeuristic(Node node1, Node node2) {
		return (Math.abs(node1.getRowIndex()-node2.getRowIndex()) + Math.abs(node1.getColumnIndex()-node2.getColumnIndex()))*10;
	}
